...
{
   Gstat = "start"
   document.getElementById("status").innerHTML = "Its your favorite part of every game guys, the loading part!"
 //Update the h3 tag which we have created inside index.html file in project 138 who has id “status” to "Game Is Loaded”.
}

function draw(){
if  (Gstat == "start") // inside the if condition check if the game_status is equal to the value "start".
{
  background(0); 
...
if(pcscore ==4){
    ...
    text("Press Restart Button to Play Agian :)!!!",width/2,height/2+30)
    noLoop();
    pcscore = 0;
 }
...
function preload()
{
  ballTouchPaddle = loadSound("ball_touch_paddle.wav")
